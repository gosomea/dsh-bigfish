...
export class Speech {
 private category:Category|null=null;private template='';private changedAt=-Infinity;private selectedAt=-Infinity;private bridge=false;private last=new Map<Category,number>();private session='';
 constructor(private random=()=>Math.random()){}
 choose(session:string,category:Category,lines:string[],now:number,minSeconds:number,level:number,values:Record<string,string|number>){
  if(session!==this.session){this.session=session;this.category=null;this.changedAt=-Infinity;this.selectedAt=-Infinity;this.bridge=false;this.last.clear();}
  ...
  if(changed&&!urgent&&this.category&&!['idle','start','waiting','error','complete','stopped'].includes(this.category)&&now-this.changedAt<minSeconds*1000){
   this.category=category;this.template='我在继续处理～';this.bridge=true;
   ...
   this.template=lines[index]??'';this.last.set(category,index);this.category=category;this.changedAt=this.selectedAt=now;this.bridge=false;
  }
