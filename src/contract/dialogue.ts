...
export const defaultLines: Record<Category,string[]> = {
 ...
 read:['我去文件里翻翻线索～','正在认真读 {file}～','线索可能藏在这里！'],search:['让我找找看……','目标锁定，开始搜寻！'],edit:['这里修一下～','正在认真改作业！'],command:['命令跑起来啦！','我盯着执行结果呢～'],test:['检查一下有没有小问题！','正在逐项检查～'],web:['我去网上看看～','正在翻阅网页～'],parallel:['分头忙起来啦！','正在并行处理 {activeCount} 项～'],tool:['工具箱拿来啦！','正在使用 {tool}～'],waiting:['这一步要你点头哦～','我在这里等你确认～'],error:['这一步出了点问题……','让我看看哪里卡住了～'],complete:['做好啦，来看看！','完成！给我放个小假吧～'],stopped:['已经停下啦～'],image:['正在画画，稍等一下～'],export:['正在整理交付文件～'],background:['后台任务还在忙哦～'],agent:['小伙伴们一起开工啦！'] };
...
export function parseLines(raw: string): Partial<Record<Category,string[]>> {
 ...
 for(const [key,lines] of Object.entries(v))if(!categories.includes(key as Category)||!Array.isArray(lines)||lines.length>30||lines.some(x=>typeof x!=='string'||x.length>160||!x.trim()||/\{(?!tool\}|file\}|elapsed\}|activeCount\}|completedCount\})[^}]*\}/.test(x)))throw Error('台词或占位符无效');
...
export function formatLine(line:string, values:Record<string,string|number>):string {
 return line.replace(/\{(tool|file|elapsed|activeCount|completedCount)\}/g,(_,key)=>String(values[key]??({file:'这个文件',tool:'工具',elapsed:'片刻',activeCount:1,completedCount:0} as Record<string,string|number>)[key])).slice(0,240);
}
