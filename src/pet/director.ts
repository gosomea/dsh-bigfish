...
export interface PetInput {state:Snapshot['state'];tag?:string;richness:number;reduced:boolean;idle?:IdlePresentation|undefined;activity:number;preview?:string;pressure:number;whipHz?:number}
...
export class PetDirector {
 ...
 private pending=''; private pendingAt=0; private recent:string[]=[]; private played=new Map<string,number>();
 ...
 choose(input:PetInput,now:number):{animation:PetAnimation;time:number;still:boolean}{
  ...
  if(current&&this.last!==undefined)this.playhead+=Math.max(0,now-this.last)*playbackSpeed(current,input.activity);
  this.last=now;
  ...
  const boundary=!current||(current.loop?Math.floor(previousTime/duration)!==Math.floor(this.playhead/duration):this.playhead>=duration);
  ...
  const rotate=!quiet&&!still&&!explicit&&now-this.since>=(input.richness===2?6000:12000);
  ...
  const overdue=changed&&now-this.pendingAt>=(current?.loop?1200:6000);
  if((changed&&(immediate||boundary||overdue))||(rotate&&boundary)){
   ...
   if(next.id!==current?.id){this.current=next;this.playhead=0;this.since=now;this.played.set(next.id,now);this.recent=[...this.recent.filter(id=>id!==next.id),next.id].slice(-3);}
