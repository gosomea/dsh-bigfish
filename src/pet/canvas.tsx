...
export function PetCanvas({loaded,input,prefs,paused=false}:{loaded:LoadedPet;input:PetInput;prefs:Preferences;paused?:boolean}){
 ...
 useEffect(()=>{
  ...
  const draw=(now:number)=>{frame=0;if(disposed||document.hidden)return;if(now-last<1000/30){frame=requestAnimationFrame(draw);return;}last=now;
   ...
   const selected=director.choose(i,now);let a=selected.animation,f=frameAt({...a,speed:[1,1]},selected.time,0,selected.still),m=loaded.pet.manifest;
   ...
   el.dataset.bounds=JSON.stringify([x,y,width,height]);el.dataset.swingStyle='indigo';el.dataset.pet=m.id;el.dataset.motion=a.id;el.dataset.frame=String(a.frames.indexOf(f));el.dataset.reduced=String(i.reduced);el.dataset.idleActive=String(Boolean(i.idle?.active));el.dataset.whip=String(motion.visible);el.dataset.actionCount=String(loaded.pet.animations.length);
