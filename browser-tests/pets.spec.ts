...
test('new role honors quiet idle, real frame changes, reduced mode, drag and icon',async({page})=>{
 await page.goto('/');await install(page);await page.getByLabel('动作表现',{exact:true}).selectOption('2');await page.getByRole('button',{name:'关闭',exact:true}).click();await page.getByRole('button',{name:'展开大肥鱼',exact:true}).click();const widget=page.getByTestId('bigfish-widget'),canvas=widget.locator('canvas');await expect(canvas).toHaveAttribute('data-pet','bigfish-adult');
