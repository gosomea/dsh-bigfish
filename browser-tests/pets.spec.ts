...
test('external role keeps identical bounds across air-swing and motion settings',async({page})=>{
 ...
 await page.getByLabel('动作表现',{exact:true}).selectOption('2');await expect(page.getByLabel('鞭策节奏',{exact:true})).toBeEnabled();await expect(page.getByTestId('motion-availability')).toContainText('8 类候选动作');await expect(canvas).toHaveAttribute('data-bounds',bounds!);
